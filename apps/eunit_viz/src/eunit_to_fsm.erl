...
%% API
-export([dynamic/2, dynamic/3, static/2, file/1, file/2, visualize/1]).
...
dynamic(FileName, MFList) ->
    Filter = fun({M,F,A}) ->
		     lists:foreach(fun({Mt, Ft}) when {M,F}=/={Mt, Ft} -> {M,F,A} end, MFList),
		     {M,F,A}
	     end,
    dynamic(FileName,[debug_on],Filter).
