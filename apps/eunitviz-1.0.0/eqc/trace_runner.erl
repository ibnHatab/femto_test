...
%    io:format("~p~n",[Traces]),
    {Pos, Neg} = lists:foldl(fun (Trace, {P, N}) ->
     			case posneg(Trace) of
			     {T, []} -> { [T| P], N};
			     {T, _} -> {P, [T| N]}
     			end
		end, {[], []}, Traces),
    print_traces(Pos, Neg),
    io:format("----------------------------------------~n"),
    {Pos, Neg}.

print_traces(Pos, Neg) ->
    SPos=lists:map(fun(HP) -> {'+', [F || {_M,F,_A} <- HP]} end, Pos),
    SNeg=lists:map(fun(HP) -> {'-', [F || {_M,F,_A} <- HP]} end, Neg),

    Run = lists:keysort(2, SPos ++ SNeg),
    lists:foreach(fun({S, Seq}) ->
			  io:format("~p ~p~n", [S, Seq])
		  end, lists:reverse(Run)).
